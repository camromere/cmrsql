...
(SELECT filename FROM sysaltfiles WHERE DB_NAME(dbid) = DB.name AND groupid!=0 AND filename LIKE '%.mdf') AS [File]
--,CASE cmptlevel
...
--		 WHEN name IN('PearlandData') THEN 'RCSQL'
--		 WHEN name IN('Su') THEN 'All_Except_RCSQL'
--		 WHEN name IN('System_Databases_Backup_Full') THEN 'System Databases'
--		 ELSE name
--END AS dbName,
...
FROM msdb.dbo.sysschedules
WHERE name LIKE '%backup%' AND name IN('RCSQL_Backup_Diff', 'All_But_RCSQL_Backup_Diff')
...
fullBackuptime=(SELECT fullBackuptime FROM #cteFull WHERE dbName='RCSQL' AND fullBackupdays IS NOT NULL),
diffBackuptime=(SELECT diffBackuptime FROM #cteDiff WHERE dbName='RCSQL' AND fullBackupdays IS NOT NULL)
