...
IF ((SELECT [VALUE] FROM ##CE WHERE [DESCRIPTION] = 'Continue') = 1)
      BEGIN
            ...
            DECLARE PUBLISHERCURSOR CURSOR LOCAL READ_ONLY FOR
                  SELECT DISTINCT https://protect-us.mimecast.com/s/R8F7CwpyNQCZ9MPiQQ37O?domain=s.name, PUB.publisher_id FROM sys.servers (NOLOCK) S JOIN dbo.MSpublications (NOLOCK) PUB
                                          ...
                                          BEGIN
                                                IF @DISTRIBUTORSERVERname = @PUBLISHERname
                                                      BEGIN
                                                            ...
                                                            SET @STMT = @STMT + 'SELECT @NUMART = COUNT(MA.objid) FROM ' + @PUBLICATIONDATABASE + '.dbo.sysmergearticles (NOLOCK) MA JOIN ' + @PUBLICATIONDATABASE + '.dbo.sysmergepublications (NOLOCK) MP ON MA.pubid = MP.pubid WHERE MP.publisher_db = ''' + @PUBLICATIONDATABASE + ''' AND https://protect-us.mimecast.com/s/MumxCxkz0RUqw6BtAgddi?domain=mp.name = ''' + @PUBLICATIOname + '''' + CHAR(13)
                                          ...
                                          BEGIN
                                                ...
                                                ELSE
                                                      BEGIN
                                                            ...
                                                            SET @STMT = @STMT + 'WHERE MP.publisher_db = ''' + @PUBLICATIONDATABASE + ''' AND https://protect-us.mimecast.com/s/MumxCxkz0RUqw6BtAgddi?domain=mp.name = ''' + @PUBLICATIOname + ''') A' + CHAR(13)
                                                            SET @STMT = @STMT + 'INSERT INTO ##CE VALUES (''Reserved'', @Reserved)' + CHAR(13)
                                                            SET @STMT = @STMT + 'INSERT INTO ##CE VALUES (''Used'', @Used)' + CHAR(13)
                                                            SET @STMT = @STMT + 'INSERT INTO ##CE VALUES (''Index'', @Index)' + CHAR(13)
                                                            EXEC (@STMT)
                                    ...
                                    IF @PUBLICATIONTYPE < 2
                                          BEGIN
                                                DECLARE SUBSCRIPTIONCURSOR CURSOR LOCAL READ_ONLY FOR
                                                      SELECT DISTINCT https://protect-us.mimecast.com/s/R8F7CwpyNQCZ9MPiQQ37O?domain=s.name, SUB.subscriber_db, SUB.subscription_type
