...
INSERT INTO #cteFull
SELECT DISTINCT 
CASE WHEN name IN('SBP Full backup EMSmarts Forms HDSQL') THEN 'EMSmarts-FORMS-HDSQL'
	 WHEN name IN('SBP Full backup RCSQL and RCSQL_Views') THEN 'RCSQLs'
	WHEN name IN('SBP Full backup System DBs') THEN 'SystemDatabases'
	WHEN name IN('SBP Full backup: RCSQL_Archive') THEN 'RCSQL_Archive'
	WHEN name IN('SBP Full backup Except RCSQLs, EMSmarts, Forms, HDSQL') THEN 'MiscDBs'
END AS dbName,
...
SELECT DISTINCT CASE 
		 WHEN name IN('SBP Diff Backup All Except RCSQLs, EMSmarts, Forms, HDSQL') THEN 'MiscDBs'
		 WHEN name IN('SBP Diff Backup EMSmarts, HDSQL, Forms') THEN 'EMSmarts-FORMS-HDSQL'
		 WHEN name IN('SBP Diff backup RCSQLs') THEN 'RCSQLs'
		 WHEN name IN('SBP Diff Backup System DBs') THEN 'SystemDatabases'
		 WHEN name IN('SBP Diff Backup: RCSQL_Archive') THEN 'RCSQL_Archive'
END AS dbName,
...
UPDATE #backups
SET fullBackupdays=(SELECT fullBackupdays FROM #cteFull WHERE dbName='RCSQLs' AND fullBackupdays IS NOT NULL),
diffBackupdays=(SELECT diffBackupdays FROM #cteDiff WHERE dbName='RCSQLs' AND diffBackupdays IS NOT NULL),
fullBackuptime=(SELECT fullBackuptime FROM #cteFull WHERE dbName='RCSQLs' AND fullBackupdays IS NOT NULL),
diffBackuptime=(SELECT diffBackuptime FROM #cteDiff WHERE dbName='RCSQLs' AND diffBackuptime IS NOT NULL)
WHERE #backups.dbName IN('RCSQL','RCSQL_Views')

UPDATE #backups
SET fullBackupdays=(SELECT fullBackupdays FROM #cteFull WHERE dbName='EMSmarts-FORMS-HDSQL' AND fullBackupdays IS NOT NULL),
diffBackupdays=(SELECT diffBackupdays FROM #cteDiff WHERE dbName='EMSmarts-FORMS-HDSQL' AND diffBackupdays IS NOT NULL),
fullBackuptime=(SELECT fullBackuptime FROM #cteFull WHERE dbName='EMSmarts-FORMS-HDSQL' AND fullBackupdays IS NOT NULL),
diffBackuptime=(SELECT diffBackuptime FROM #cteDiff WHERE dbName='EMSmarts-FORMS-HDSQL' AND diffBackuptime IS NOT NULL)
WHERE #backups.dbName IN('EMSmart', 'EMSmart_Dev', 'EMSmart_Local', 'EMSmart_Stage_LOS', 'EMSmart_Unit_Test', 'EMSmart_Stage2', 'HDSQL', 'FORMS')

UPDATE #backups
SET fullBackupdays=(SELECT fullBackupdays FROM #cteFull WHERE dbName='RCSQL_Archive' AND fullBackupdays IS NOT NULL),
diffBackupdays=(SELECT diffBackupdays FROM #cteDiff WHERE dbName='RCSQL_Archive' AND diffBackupdays IS NOT NULL),
fullBackuptime=(SELECT fullBackuptime FROM #cteFull WHERE dbName='RCSQL_Archive' AND fullBackupdays IS NOT NULL),
diffBackuptime=(SELECT diffBackuptime FROM #cteDiff WHERE dbName='RCSQL_Archive' AND diffBackuptime IS NOT NULL)
WHERE #backups.dbName IN('RCSQL_Archive')

UPDATE #backups
SET fullBackupdays=(SELECT fullBackupdays FROM #cteFull WHERE dbName='SystemDatabases' AND fullBackupdays IS NOT NULL),
diffBackupdays=(SELECT diffBackupdays FROM #cteDiff WHERE dbName='SystemDatabases' AND diffBackupdays IS NOT NULL),
fullBackuptime=(SELECT fullBackuptime FROM #cteFull WHERE dbName='SystemDatabases' AND fullBackupdays IS NOT NULL),
diffBackuptime=(SELECT diffBackuptime FROM #cteDiff WHERE dbName='SystemDatabases' AND diffBackuptime IS NOT NULL)
WHERE #backups.dbName IN('master', 'model', 'msdb')

UPDATE #backups
SET fullBackupdays=(SELECT fullBackupdays FROM #cteFull WHERE dbName='MiscDBs' AND fullBackupdays IS NOT NULL),
diffBackupdays=(SELECT diffBackupdays FROM #cteDiff WHERE dbName='MiscDBs' AND diffBackupdays IS NOT NULL),
fullBackuptime=(SELECT fullBackuptime FROM #cteFull WHERE dbName='MiscDBs' AND fullBackupdays IS NOT NULL),
diffBackuptime=(SELECT diffBackuptime FROM #cteDiff WHERE dbName='MiscDBs' AND diffBackuptime IS NOT NULL)
WHERE #backups.dbName IN('Redix', 'Amazon_ABC', 'RCSQL_Views_Reports', 'PPAPI', 'RNB_Sup', 'SSISDB', 'EMS_DebtSetoff', 'AppXtender', 'AppXtender_OTG', 'AppXtender_PROD', 'MoveIT', 'EMS_835Automation', 'ANSI837', 'EDI_formats', 'EC_EligibilityMagic', 'electroniclaim', 'DataStore', 'SSRS', 'SSRSTempDB', 'EligibillMagic', 'Perfmon', 'Admin', 'RedGate', 'BatchHistory', 'PayorLogic', 'AutomateBPA')
